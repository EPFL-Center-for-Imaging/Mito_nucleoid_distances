...
def segment_img_with_nellie(
    ...
    file_info = nellie.im_info.verifier.FileInfo(
        str(img_path.resolve()), output_dir=output_dirpath
    )
...
def calculate_distances_to_next(arc_lengths, peak_indices):
    distances = np.zeros(len(peak_indices))
...
if __name__ == "__main__":
    ...
    for folder in pathlib.Path(
        "C:/Users/landoni/Desktop/Pearling Desktop/siRNA_GA_crops/Test"
    ).glob("*"):
        img_path = folder / f"{folder.stem}.tif"
        ...
        label_path = (
            nellie_output_dir
            / "nellie_necessities"
            / f"{img_path.stem}-ZCYX-Z0p2_Y0p056_X0p056-ch{ch}-im_skel.ome.tif"
        )
        ...
        for branch_id, spline_um in enumerate(splines_um):
            ...
            arc_lengths_um = spline_um.arc_length(t)
